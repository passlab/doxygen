                      ...
                      const QCString &text)
{
  ...
  auto ast    { validatingParseDoc(*parser.get(),
                                   fileName,lineNr,scope,mdext,FALSE,FALSE,
...
static void llmDocGenForMember(const MemberDef *md,TextStream &ti,TextStream &t,const Definition *def)
{
  ...
  //enum { define_t,variable_typedef_t,enum_t,function_t } xmlType = function_t;
     ...
     )
  {
    ...
    t << "        <type>";
    linkifyText(TextGeneratorXMLImpl(t),def,md->getBodyDef(),mdypeStr);
    t << "</type>\n";
    if (md->isTypeAlias())
    {
...
static void llmDocGenSection(const Definition *d,TextStream &ti,TextStream &t,
...
static void llmDocGenForClass(const ClassDef *cd)
{
  // + brief description
  ...
  msg("LLMDOCGEN: Generating doc for class %s\n",qPrint(cd->name()));

  classOutputFileBase(cd);
  cd->compoundTypeString();
  convertToXML(cd->name());
  langToString(cd->getLanguage());

  switch (cd->protection())
  {
    case Protection::Public:    ;   break;
    case Protection::Protected: ;   break;
    case Protection::Private:   ;   break;
    case Protection::Package:   ;   break;
  }
  if (cd->isFinal()) ;
  if (cd->isSealed()) ;
  if (cd->isAbstract()) ;
  ...
  writeIncludeInfo(cd->includeInfo());

  writeInnerClasses(cd->getClasses());

  writeTemplateList(cd);
  for (const auto &mg : cd->getMemberGroups())
  {
    llmDocGenSection(cdi,&mg->members(),"user-defined",mg->header(),
        mg->documentation());
  }
  ...
  for (const auto &ml : cd->getMemberLists())
  {
    ...
    {
      llmDocGenSection(cdi,ml.get(),ml->listType().toXML());
    }
  }
...
static void llmDocGenForConcept(const ConceptDef *cd,TextStream &ti)
{
...
static void llmDocGenForModule(const ModuleDef *mod,TextStream &ti)
{
  ...
  t << "</compoundname>\n";
  writeInnerFiles(mod->getUsedFiles());
  writeInnerClasses(mod->getClasses());
  writeInnerConcepts(mod->getConcepts());
  for (const auto &ml : mod->getMemberLists())
  {
    if (ml->listType().isDeclaration())
    {
      llmDocGenSection(modi,ml.get(),ml->listType().toXML());
    }
  }
  for (const auto &mg : mod->getMemberGroups())
  {
    llmDocGenSection(modi,&mg->members(),"user-defined",mg->header(),
        mg->documentation());
  }
  t << "    <briefdescription>\n";
...
static void llmDocGenForNamespace(const NamespaceDef *nd,TextStream &ti)
{
  ...
  writeInnerClasses(nd->getClasses());
  writeInnerConcepts(nd->getConcepts());
  writeInnerNamespaces(nd->getNamespaces());

  for (const auto &mg : nd->getMemberGroups())
  {
    llmDocGenSection(ndi,&mg->members(),"user-defined",mg->header(),
          mg->documentation());
  }
  ...
  for (const auto &ml : nd->getMemberLists())
  {
    if (ml->listType().isDeclaration())
    {
      llmDocGenSection(ndi,ml.get(),ml->listType().toXML());
    }
  }
...
static void llmDocGenForFile(FileDef *fd,TextStream &ti)
{
  // + includes files
  // + includedby files
  // + include graph
  ...
  writeInnerConcepts(fd->getConcepts());
  writeInnerNamespaces(fd->getNamespaces());

  for (const auto &mg : fd->getMemberGroups())
  {
    llmDocGenSection(fdi,&mg->members(),"user-defined",mg->header(),
        mg->documentation());
  }
  ...
  for (const auto &ml : fd->getMemberLists())
  {
    if (ml->listType().isDeclaration())
    {
      llmDocGenSection(fdi,ml.get(),ml->listType().toXML());
    }
  }
...
static void llmDocGenForGroup(const GroupDef *gd,TextStream &ti)
{
  // + members
  // + member groups
  // + files
  ...
  writeInnerModules(gd->getModules());
  writeInnerFiles(gd->getFiles());
  writeInnerClasses(gd->getClasses());
  writeInnerConcepts(gd->getConcepts());
  writeInnerNamespaces(gd->getNamespaces());
  writeInnerPages(gd->getPages());
  writeInnerGroups(gd->getSubGroups());

  for (const auto &mg : gd->getMemberGroups())
  {
    llmDocGenSection(gdi,&mg->members(),"user-defined",mg->header(),
        mg->documentation());
  }
  ...
  for (const auto &ml : gd->getMemberLists())
  {
    if (ml->listType().isDeclaration())
    {
      llmDocGenSection(gdi,ml.get(),ml->listType().toXML());
    }
  }
...
static void llmDocGenForDir(DirDef *dd,TextStream &ti)
{
  ...
  writeInnerDirs(&dd->subDirs());
  writeInnerFiles(dd->getFiles());
...
static void llmDocGenForPage(PageDef *pd,TextStream &ti,bool isExample)
{
  // + name
  // + title
  // + documentation
  ...
  writeInnerPages(pd->getSubPages());
